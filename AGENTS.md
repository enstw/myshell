...
2. `scripts/install` — bash 5. The source of truth. Gathers all interactive answers up front (recorded under `~/.local/state/myshell/` — the decision, not system state, is the idempotency key), then runs six phases in a fixed order: **system** (locale, timezone, passwordless apt/snap, Ubuntu Pro/ESM notices off), **shell** (zsh + prompt + plugins, the generated dotfiles, `~/bin/myshell-update` + `update.d/*.sh`, git identity, `chsh`), **fonts**, **packages** (the CLI toolbox, tealdeer, docker group, the optional-apps menu, purge, unminimize), **runtimes** (uv, pnpm + Node LTS, bun), **agents** (the chosen AI agents + CodeGraph). `--phase LIST` runs a subset; `--list-phases` prints them. Read its header banner for the design rules (phases and their ordering constraints, Python-via-uv-only, locale, timezone, the AI-agent and optional-app menus, zsh file layout).
...
- myshell self-update: `myshell-update`'s first section compares `~/.local/state/myshell/installed-commit` (written by `record_installed_source` at the end of a **full** install run) against origin's HEAD, and on a mismatch re-runs the canonical curl|sh path — download the tarball, run its `bootstrap` — then `exec`s the freshly deployed `~/bin/myshell-update` so the rest of the sweep uses the new code. It never `git pull`s: a recorded `checkout` path is read only to spot a maintainer box whose build is *ahead* of origin, which is left alone. `MYSHELL_NO_SELF_UPDATE=1` opts out; `MYSHELL_SELF_UPDATED=1` is the internal re-exec loop guard. The repo URL is spelled in `bootstrap`, `scripts/install`, and `scripts/myshell-update` for the same reason the output helpers are — `scripts/check` pins the three against each other.
