...
1. `configure_terminal_profile_macos` re-opened Terminal on every re-run — **fixed 2026-09-01**. Diagnosed from run 33410797061: the fresh run set `Default Window Settings`, the re-run 43s later could not see it and re-imported, a third run 1.5s after that one could — a cold-launching Terminal writes its own preferences *after* our `defaults write` at `sleep 1` and clobbers it. The import now keys on whether the profile exists (not on whether it is the default) and polls for Terminal's own write instead of sleeping; run 33412230146 confirms the fresh run reads back correctly and both re-runs report "already default — skipping". One gap remains, which is why this stays here: nothing detects Terminal *later* clobbering the value when it quits — the install-time read-back cannot see that.
