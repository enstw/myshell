...
| `.github/workflows/ci.yml` | CI: `scripts/check` + `scripts/test-prompts` + both round-trips (sudo user and root) in minimized `ubuntu:26.04` containers, plus a macOS-runner round-trip (brew + purge paths). |
